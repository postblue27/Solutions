...
std::stack<Node*> BuildTree(std::string inputString, std::stack<Node*> operands, std::stack<Node*> operators) {
	...
	for (int i = 0; i < inputString.length(); i++) {
		...
		if (IsOperator(std::string(1, inputString[i])) || inputString[i] == '(' || inputString[i] == ')') {
			...
			if (IsOperator(std::string(1, inputString[i]))) {
				while (true) {
					...
					else if ((priority(std::string(1, inputString[i])) > priority(operators.top()->value)) || (priority(std::string(1, inputString[i])) == priority(operators.top()->value))) {
						operators.push(NewNode(std::string(1, inputString[i])));
						break;
					}
